...
def render_deal_deep_dive(deals, timelines, coaching_cards_data):
    ...
    st.caption(
        "Inspect seller behaviors, interaction sequences, and evidence-based recommendations."
    )

    # Deal Selector Dropdown
    deal_options = {
        d["id"]: f"{d['name']} — ${d['amount']:,.0f} ({d['risk_level']} Risk)"
        for d in deals
    }
    selected_deal_id = st.selectbox(
        "Select Opportunity to Inspect",
    ...
    with col2:
        st.metric(
            "Current Stage",
            selected_deal["stage"],
            f"{selected_deal['days_in_stage']} days",
        )
    with col3:
    ...
    with col4:
        st.metric(
            "Risk Score",
            f"{selected_deal['risk_score']} / 100",
            delta=selected_deal["risk_level"],
            delta_color="inverse",
        )
