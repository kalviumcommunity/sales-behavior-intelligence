...
def render_rep_coaching(reps, deals):
    ...
    st.caption(
        "Identify individual seller habits, strengths, and repeatable coaching playbooks."
    )

    selected_rep_name = st.selectbox(
        "Select Sales Representative", [r["name"] for r in reps]
    )
    ...
    with col3:
        st.metric(
            "High Risk Deals",
            len(high_risk_rep_deals),
            delta="Needs Coaching" if high_risk_rep_deals else "On Track",
            delta_color="inverse",
        )
    ...
    if selected_rep_name == "Maya Lin":
        st.info(
            "💡 **Key Behavioral Insight for Maya**: Maya excels at initial technical discovery and building rapport with technical champions. However, data reveals a pattern of **post-demo proposal follow-up lag** (>48 hours) and **single-threading** on enterprise accounts."
        )
        st.markdown("""
        ...
        st.success(
            f"💡 **Key Behavioral Insight for {selected_rep_name}**: Maintains strong follow-up cadence and consistent next-step scheduling across deals."
        )
