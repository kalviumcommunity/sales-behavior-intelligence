...
def render_auth_page():
    st.html(
        "<h2 style='text-align: center;'>Welcome to Sales Behavior Intelligence</h2>"
    )
    st.html(
        "<p style='text-align: center; color: #6b7280;'>Please log in or sign up to access your dashboard.</p>"
    )
        ...
        with tab1:
            with st.form("login_form"):
                ...
                username = st.text_input(
                    "Username", placeholder="e.g. admin24 or user123"
                )
                ...
                if submit_button:
                    if not username or not password:
                        ...
                        st.success(
                            f"Successfully logged in as {st.session_state.role}!"
                        )
                        st.rerun()
