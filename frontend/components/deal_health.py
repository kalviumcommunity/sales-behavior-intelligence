...
def render_deal_health(health_metrics):
    ...
    with cols[0]:
        ...
        color = (
            "#5fd6a0"
            if score_pct >= 70
            else "#ffb76a" if score_pct >= 50 else "#ff8ea7"
        )
    ...
    with cols[1]:
        risk_color = (
            "#ff8ea7"
            if health_metrics["risk_level"] == "High"
            else "#ffb76a" if health_metrics["risk_level"] == "Medium" else "#5fd6a0"
        )
    ...
    with cols[2]:
        velocity_color = (
            "#ff8ea7" if "Below" in health_metrics["deal_velocity"] else "#ffb76a"
        )
    ...
    with cols[3]:
        next_step_status = (
            "✓ Confirmed"
            if health_metrics["next_step_confirmed"]
            else "✗ Not Confirmed"
        )
        next_step_color = (
            "#5fd6a0" if health_metrics["next_step_confirmed"] else "#ffb76a"
        )
