...
def create_deal(deal_in: DealCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """Create a deal."""
    try:
...
def get_deal(deal_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """Get a deal."""
...
def update_deal(deal_id: str, deal_in: DealUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """Update a deal."""
    try:
...
def delete_deal(deal_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    """Delete a deal."""
