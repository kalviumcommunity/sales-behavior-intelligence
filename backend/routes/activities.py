...
def create_activity(
    ...
    try:
        return ActivityResponse.model_validate(
            ActivityService(db).create_activity(activity_in)
        )
...
def list_activities(
    ...
    return [
        ...
        for activity in ActivityService(db).list_activities(skip, limit)
    ]


@router.get("/deal/{deal_id}", response_model=list[ActivityResponse])
def get_activities_by_deal(
    deal_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Get activities for a deal."""
    return [
        ActivityResponse.model_validate(activity)
        for activity in ActivityService(db).get_activities_by_deal(deal_id)
    ]
