...
with nav_col:
    ...
    for s in SETTINGS_SECTIONS:
        ...
        active_style = (
            "background: var(--sbi-cyan-dim); color: var(--sbi-cyan); font-weight: 600;"
            if is_active
            else "color: var(--sbi-text-secondary);"
        )
        nav_html += f"""
    ...
    for s in SETTINGS_SECTIONS:
        if st.button(
            ...
            type=(
                "primary"
                if st.session_state.settings_section == s["id"]
                else "secondary"
            ),
        ):
...
with content_col:
    ...
    # ── Workspace ────────────────────────────────────────────
        ...
        section_header(
            "Workspace",
            "Configure your Sales Behavior Intelligence workspace settings.",
        )
    ...
    # ── Notifications ─────────────────────────────────────────
        ...
        section_header(
            "Notifications", "Control which signals and events trigger notifications."
        )
        ...
        notification_settings = [
            ...
            (
                ...
                "Receive AI-generated coaching suggestions.",
                True,
            ),
            ...
            (
                "Rep Inactivity Alerts",
                "Alert when a rep hasn't updated a deal in 5+ days.",
                True,
            ),
        ]
        ...
        for name, desc, default in notification_settings:
            ...
            with row_col2:
                ...
                st.toggle(
                    "", value=default, key=f"notif_{name.replace(' ', '_').lower()}"
                )

    # ── AI Preferences ────────────────────────────────────────
    elif section == "ai_prefs":
        section_header(
            "AI Preferences", "Tune the AI coaching engine to match your team's needs."
        )
        ...
        st.caption(
            f"Current: **{risk_level}** — AI will flag more deals as at-risk with higher sensitivity."
        )
        ...
        st.caption(
            f"Only show coaching recommendations with ≥{threshold}% AI confidence."
        )
    ...
    # ── CRM Integrations ──────────────────────────────────────
        ...
        section_header(
            "CRM Integrations", "Connect your CRM to enable automatic activity sync."
        )
        ...
        integrations = [
            {
                ...
                "status_cls": "neutral",
                "icon": "☁",
                "desc": "Connect to sync deals, activities, and contacts.",
            },
            {
                "name": "HubSpot",
                ...
                "status_cls": "neutral",
                "icon": "⬡",
                "desc": "Bi-directional sync with HubSpot CRM.",
            },
            {
                "name": "Outreach.io",
                "status": "Demo Mode",
                "status_cls": "neutral",
                "icon": "✉",
                "desc": "Analyze outreach sequences and email cadence data.",
            },
            {
                "name": "Gong",
                "status": "Not Connected",
                "status_cls": "neutral",
                "icon": "◎",
                "desc": "Import call transcripts for behavioral analysis.",
            },
        ]
    ...
    # ── Security ──────────────────────────────────────────────
        ...
        section_header(
            "Security", "Manage your password, sessions, and access controls."
        )
