...
search = fc1.text_input(
    "Search", placeholder="Search deals or companies...", label_visibility="collapsed"
)
stage_filter = fc2.selectbox(
    "Stage",
    ["All Stages", "Discovery", "Qualification", "Proposal", "Negotiation", "Contract"],
    label_visibility="collapsed",
)
risk_filter = fc3.selectbox(
    "Risk", ["All Risk", "Low", "Medium", "High"], label_visibility="collapsed"
)
rep_filter = fc4.selectbox(
    "Rep",
...
if search:
    filtered_deals = [
        ...
        if search.lower() in d["company"].lower()
        or search.lower() in d["deal_name"].lower()
    ]
...
st.html(
    f"<div class='sbi-text-muted sbi-text-xs sbi-font-semibold' style='margin-bottom: 12px;'>Showing {len(filtered_deals)} deals</div>"
)
...
for deal in filtered_deals:
    ...
    if deal["stage"] == "Closed Won":
        stage_badge_class = "success"
    elif deal["stage"] == "Closed Lost":
        stage_badge_class = "danger"
    elif deal["stage"] in ["Negotiation", "Contract"]:
        stage_badge_class = "warning"

    risk_color = "var(--sbi-success)"
    if deal["risk_level"] == "High":
        risk_color = "var(--sbi-danger)"
    elif deal["risk_level"] == "Medium":
        risk_color = "var(--sbi-warning)"
    ...
    st.html(
        "<div style='height: 1px; background: var(--sbi-border-subtle); margin: 0;'></div>"
    )

if not filtered_deals:
    render_empty_state(
        "No deals found", "Try adjusting your filters to see more results."
    )
